...
@Component({
  selector: 'app-onaylanan-stepper',
  templateUrl: './onaylanan-stepper.component.html',
  styleUrl: './onaylanan-stepper.component.css'
})
export class OnaylananStepperComponent {
  userYetki: string = "";
  userMudurluk: string = "";
  ...
  checkDataSource = new MatTableDataSource<TalepModel>();
  columns = [
    {
      columnDef: 'basvuruno',
      header: 'Başvuru No',
    ...
    const filterTalepler =  () => {  
      this.TaleplerService.filterTalepler(this.filterDataCheck).subscribe(
        (response: any) => {
          ...
          this.checkDataSource = new MatTableDataSource(values);
        },
        (error: any) => {
